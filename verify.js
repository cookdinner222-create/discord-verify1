...
// 🛠️ 인증된 유저들의 토큰을 영구 저장할 JSON 파일 경로
...
client.on('messageCreate', async (message) => {
    ...
    // 🛠️ !서버복구 입력 시 인증했던 유저들의 토큰을 꺼내서 복구 서버에 즉시 자동 참가
    if (content === '!서버복구') {
        try {
            const member = message.member;
            if (!member) return;
            ...
            if (!accessToken) {
                return message.reply('⚠️ 저장된 인증 토큰 정보를 찾을 수 없습니다. (인증을 다시 진행해 주세요!)');
            }
            ...
            message.reply('⚠️ 복구 서버 자동 참가 중 오류가 발생했습니다. (봇이 복구 서버에 관리자 권한으로 초대되어 있는지 확인해 주세요)');
        }
    }
});
...
app.get('/callback', async (req, res) => {
    ...
    try {
        ...
        // 🛠️ 인증 성공 시 유저의 액세스 토큰을 파일에 영구 저장
